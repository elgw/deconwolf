...
typedef struct{
    ...
    char * image;
    char * out;
    /* Where all the files generated by dw_nuclei should be stored */
    char * workdir;
    ...
    fimo * bg_model;
    int nsigma; // Number of sigma
    float sigma0; // First sigma
    float * sigma; // sigma values passed to feature extraction
...
static void opts_print(FILE * f, opts * s)
{
    ...
    if(s->purpose == NUC_CLASSIFY || s->purpose == NUC_FIT)
    {
        fprintf(f, "Model file: %s\n", s->modelfile);
    }
    fprintf(f, "%d Features scales:\n", s->nsigma);
    if(s->sigma != NULL)
    {
        for(int kk = 0; kk < s->nsigma; kk++)
        {
            if(kk == 0)
            {
                fprintf(f, "   %.2f", s->sigma[kk]);
            } else {
                fprintf(f, ", %.2f", s->sigma[kk]);
        }
        }
...
static char *
gen_annotation_file_name(const opts *s, const char *tifffile)
{
    if(s->workdir == NULL)
    {
        char * out = calloc(strlen(tifffile)+16, 1);
        assert(out != NULL);
        sprintf(out, "%s.a.png", tifffile);
        return out;
    }

    char * fname = dw_basename(tifffile);
    assert(fname != NULL);
    char * out = calloc(strlen(s->workdir) + strlen(fname) + 16, 1);
    assert(out != NULL);
    sprintf(out, "%s%c%s.a.png", s->workdir, FILESEP, fname);
    return out;
}

static char *
gen_prediction_file_name(const opts *s, const char *tifffile)
{
    if(s->workdir == NULL)
    {
        char * out = calloc(strlen(tifffile)+16, 1);
        assert(out != NULL);
        sprintf(out, "%s.prediction.png", tifffile);
        return out;
    }

    char * fname = dw_basename(tifffile);
    assert(fname != NULL);
    char * out = calloc(strlen(s->workdir) + strlen(fname) + 32, 1);
    assert(out != NULL);
    sprintf(out, "%s%c%s.prediction.png", s->workdir, FILESEP, fname);
    return out;
}
...
static void usage(__attribute__((unused)) int argc, char ** argv)
{
    ...
    printf(" --fout name"
           "\n\t Export features and class to npy files using name as base"
           "\n\t For debugging\n");
    printf(" --workdir path"
           "\n\t where to write/read files generated by dw nuclei\n");
    printf(" --help\n\t Show this message\n");
    opts_free(s);
}
...
static void argparsing(int argc, char ** argv, opts * s)
{
    struct option longopts[] = {
        ...
        {"nscale", required_argument, NULL, 's'},
        {"scale", required_argument, NULL, 'S'},
        {"fout", required_argument, NULL, 'f'},
        {NULL, 0, NULL, 0}};
    int ch;
    while((ch = getopt_long(argc, argv,
                            "IF:P:hot:v:123bw:s:Sf:",
                            longopts, NULL)) != -1)
    {
        switch(ch){
        case 'f':
        ...
        case 'h':
            ...
            break;
        case 's':
            s->nsigma = atoi(optarg);
            break;
        case 'S':
            s->sigma0 = atof(optarg);
            break;
        case 't':
    ...
    if(s->workdir)
    {
        if(!dw_isdir(s->workdir))
        {
            printf("The --workdir %s does not exist\n", s->workdir);
            exit(EXIT_FAILURE);
        }
    }
    if(s->nsigma < 1)
    {
        printf("--scales %d is invalid, has to be at least 1\n", s->nsigma);
    }
    free(s->sigma);
    s->sigma = calloc(s->nsigma, sizeof(float));
    assert(s->sigma != NULL);
    s->sigma[0] = s->sigma0;
    for(int kk = 1; kk < s->nsigma; kk++)
    {
        s->sigma[kk] = sqrt(2)*s->sigma[kk-1];
    }
...
get_reduction(opts * s, const char * file)
{
    ...
    if(s->redu == REDU_MEAN)
    {
        float * sumI = fim_sumproj(I, M, N, P);
        fim_free(I);
        fimo * result = fim_image_from_array(sumI, M, N, 1);
        fim_free(sumI);
        if(s->bg_model != NULL)
        {
            printf("Applying background model\n");
...
fit(opts * s, int argc, char ** argv)
{
    ...
    for(int kk = s->optpos; kk < argc; kk++)
    {
        ...
        char *name_annotation = gen_annotation_file_name(s, name_image);
        ...
        ftab_t * image_features = fim_features_2d(image, s->sigma, s->nsigma);
    ...
    if(fit_labels == NULL)
    {
        printf("Could not load any annotations\n");
        exit(EXIT_FAILURE);
    }
    ...
    if(s->fout != NULL)
    {
        printf("Exporting features to disk (--fout %s)\n", s->fout);
        char * outname = calloc(strlen(s->fout) + 32, 1);
        sprintf(outname, "%s_x.npy", s->fout);
        printf("   Writing features to %s\n", outname);
        ftab_to_npy(fit_features, outname);
        sprintf(outname, "%s_y.npy", s->fout);
        printf("   Writing labels to %s\n", outname);
        ftab_to_npy(fit_labels, outname);
        free(outname);
    }


    /* Convert features to double and class labels to u32 */
    u64 nfeature = fit_features->ncol;
    u64 nsample = fit_features->nrow;
    double * fit_features8 = ftab_get_data_f64(fit_features);
    assert(fit_features8 != NULL);
    ftab_free(fit_features);

    u32 * fit_labels4 = ftab_get_data_u32(fit_labels);
...
classify(opts * s, int argc, char ** argv)
{
    ...
    for(int kk = s->optpos; kk < argc; kk++)
    {
        ...
        char * name_out = gen_prediction_file_name(s, name_image);
...
init(opts * s, int argc, char ** argv)
{
    ...
    for(int kk = s->optpos; kk < argc; kk++)
    {
        ...
        char *out = gen_annotation_file_name(s, argv[kk]);

    ...
    __attribute__((unused)) const dw_opts * s)
{
  ...
  putdot(s);
  afloat * y = fft_convolve_cc_f2(fftPSF, F, wM, wN, wP); // F is freed
...
#pragma omp parallel for
  ...
  afloat * x = fft_convolve_cc_conj_f2(fftPSF, F_sn, wM, wN, wP);
    ...
    dw_opts * s)
{
  ...
  /* <-- This isn't needed ...
    fftwf_complex * cKr = NULL;
     ...
     cKr = fft(Zr, wM, wN, wP);
  // Not needed since f(-x) = ifft(conj(fft(f)))
  fftwf_free(Zr);
  --> */

  //printf("initial guess\n"); fflush(stdout);
  fftwf_complex * F_one = initial_guess(M, N, P, wM, wN, wP);
  afloat * P1 = fft_convolve_cc_conj_f2(fftPSF, F_one, wM, wN, wP); // can't replace this one with cK!
  //printf("P1\n");
